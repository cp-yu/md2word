...
HEADING_MARKER_RE = re.compile(r"\[\[MD2WORD_HEADING:(\d+)\]\]")
WORD_HEADING_NAME_RE = re.compile(r"^(?:heading|标题)\s*([1-9])$", re.IGNORECASE)
IGNORABLE_NAMESPACE_MAP = {
...
def main() -> int:
    ...
    try:
        ...
        normalize_docx_headings(stage_paths.raw_docx)
        normalize_docx_headings(stage_paths.processed_docx)
...
def normalize_docx_headings(docx_path: Path) -> None:
    with ZipFile(docx_path, "r") as archive:
        document_xml = archive.read("word/document.xml")
        styles_xml = archive.read("word/styles.xml")

    heading_style_ids = collect_heading_style_ids(styles_xml)
    ...
    paragraph_properties_change_tag = f"{{{WORD_NAMESPACE}}}pPrChange"
    ...
    outline_level_tag = f"{{{WORD_NAMESPACE}}}outlineLvl"
    ...
    for paragraph in root.iter(paragraph_tag):
        for run in paragraph.findall(run_tag):
            for text_node in run.findall(text_tag):
                ...
                match = HEADING_MARKER_RE.search(text)
                if match:
                    paragraph_levels.append(min(max(int(match.group(1)), 1), 6))
                    break
            else:
                continue
            break

    base_level = min(paragraph_levels, default=1)

    for paragraph in root.iter(paragraph_tag):
        level = None
        for run in list(paragraph.findall(run_tag)):
            run_text_nodes = list(run.findall(text_tag))
            if not run_text_nodes:
                continue
            for text_node in run_text_nodes:
                text = text_node.text or ""
                match = HEADING_MARKER_RE.search(text)
                if not match:
                    continue
                level = min(max(int(match.group(1)), 1), 6)
                text_node.text = HEADING_MARKER_RE.sub("", text)
            removable_children = [
                child
                for child in list(run)
                if child.tag not in {run_properties_tag, text_tag, last_rendered_page_break_tag}
            ]
            if all(not (text_node.text or "") for text_node in run_text_nodes) and not removable_children:
                paragraph.remove(run)

        if level is None:
            continue

        normalized_level = min(max(level - base_level + 1, 1), 6)
        paragraph_properties = paragraph.find(paragraph_properties_tag)
        if paragraph_properties is None:
            paragraph_properties = ET.Element(paragraph_properties_tag)
            paragraph.insert(0, paragraph_properties)

        paragraph_style = paragraph_properties.find(paragraph_style_tag)
        style_id = heading_style_ids.get(normalized_level)
        if style_id:
            if paragraph_style is None:
                paragraph_style = ET.Element(paragraph_style_tag)
                insert_paragraph_property_before(
                    paragraph_properties,
                    paragraph_style,
                    {section_properties_tag, paragraph_properties_change_tag},
                    default_index=0,
                )
            paragraph_style.set(value_key, style_id)

        outline_level = paragraph_properties.find(outline_level_tag)
        if outline_level is None:
            outline_level = ET.Element(outline_level_tag)
            insert_paragraph_property_before(
                paragraph_properties,
                outline_level,
                {run_properties_tag, section_properties_tag, paragraph_properties_change_tag},
            )
        outline_level.set(value_key, str(normalized_level - 1))

    updated_document_xml = ET.tostring(root, encoding="utf-8", xml_declaration=True)
    updated_document_xml = restore_ignorable_namespace_declarations(root, updated_document_xml)

    with tempfile.NamedTemporaryFile(dir=docx_path.parent, delete=False, suffix=".docx") as temp_file:
        temp_path = Path(temp_file.name)

    try:
        with ZipFile(docx_path, "r") as source_archive, ZipFile(
            temp_path,
            "w",
...
def collect_heading_style_ids(styles_xml: bytes) -> dict[int, str]:
    root = ET.fromstring(styles_xml)
    style_tag = f"{{{WORD_NAMESPACE}}}style"
    name_tag = f"{{{WORD_NAMESPACE}}}name"
    type_key = f"{{{WORD_NAMESPACE}}}type"
    style_id_key = f"{{{WORD_NAMESPACE}}}styleId"
    value_key = f"{{{WORD_NAMESPACE}}}val"
    heading_style_ids: dict[int, str] = {}

    for style in root.findall(style_tag):
        if style.get(type_key) != "paragraph":
            continue
        style_name_node = style.find(name_tag)
        if style_name_node is None:
            continue
        style_name = (style_name_node.get(value_key) or "").strip()
        match = WORD_HEADING_NAME_RE.match(style_name)
        if not match:
            continue
        level = int(match.group(1))
        style_id = style.get(style_id_key)
        if style_id:
            heading_style_ids[level] = style_id

    return heading_style_ids


def insert_paragraph_property_before(
    ...
    *,
    ...
    for index, existing_child in enumerate(list(paragraph_properties)):
        if existing_child.tag in before_tags:
            paragraph_properties.insert(index, child)
            return

    if default_index is not None:
        paragraph_properties.insert(default_index, child)
        return

    paragraph_properties.append(child)


def restore_ignorable_namespace_declarations(root: ET.Element, document_xml: bytes) -> bytes:
    ignorable_value = root.get(f"{{{MARKUP_COMPATIBILITY_NAMESPACE}}}Ignorable", "").strip()
    ...
    for prefix in ignorable_value.split():
        namespace_uri = IGNORABLE_NAMESPACE_MAP.get(prefix)
    ...
    root_start = xml_text.find("<", declaration_end + 2 if declaration_end >= 0 else 0)
    if root_start < 0:
        return document_xml

    document_tag_end = xml_text.find(">", root_start)
    if document_tag_end < 0:
        return document_xml

    insertion = " " + " ".join(missing_declarations)
    xml_text = xml_text[:document_tag_end] + insertion + xml_text[document_tag_end:]
